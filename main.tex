...
\section{问题定义}
赛题是一个典型的图像分类问题，即根据一定的分类规则将图像自动分到一组预定义类别中。图像分类在多领域有着广泛的实际应用。实际应用中，图像分类任务会受到视角的变化，大小的变化，物体的形变，遮挡，光照条件，\enlargethispage{-3.3cm}背景干扰和多种子类型等各方面因素\citeBUAA{cs231n}的挑战。
